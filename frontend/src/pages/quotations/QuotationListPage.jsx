...
export default function QuotationListPage() {
  ...
  const fetchQuotations = useCallback(async () => {
    ...
    try {
      ...
      computeSummary(data || [], totalCount || 0);
    } catch (err) {
  ...
  // Compute summary values from data
  function computeSummary(data, totalCount) {
    let draftCount = 0;
    let sentCount = 0;
    let acceptedCount = 0;
    let declinedCount = 0;
  ...
  const sortedQuotations = useMemo(() => {
    ...
    switch (clientSortBy) {
      case 'date_asc': return arr.sort((a, b) => new Date(a.quotation_date||a.created_at||0) - new Date(b.quotation_date||b.created_at||0));
      case 'date_desc': return arr.sort((a, b) => new Date(b.quotation_date||b.created_at||0) - new Date(a.quotation_date||a.created_at||0));
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleting(true);
    try {
      await apiClient.delete(`/quotations/${deleteConfirm.id}`);
      toast.success(`Quotation ${deleteConfirm.quotation_number} deleted successfully.`);
  ...
  return (
    <div>
      ...
      <div className="bg-white border-b border-[var(--zoho-border)] px-6 py-4 -mx-6 -mt-6 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-semibold text-[var(--zoho-text)]">Quotations</h1>
            {!loading && (
      ...
      <ZohoPaymentBanner
        items={[
          { label: 'Total Quotations', value: String(summary.totalQuotations), color: '#0071DC' },
          { label: 'Draft', value: String(summary.draftCount), color: '#6B7280' },
          { label: 'Sent', value: String(summary.sentCount), color: '#3B82F6' },
          { label: 'Accepted', value: String(summary.acceptedCount), color: '#16A34A' },
          { label: 'Declined', value: String(summary.declinedCount), color: '#DC2626' },
          { label: 'Total Value', value: formatINR(summary.totalValue), color: '#F59E0B' },
        ]}
      />
      ...
      <div className="bg-white rounded-lg border border-[var(--zoho-border)] overflow-hidden">
        ...
        {loading ? (
          <div className="py-20">
            <LoadingSpinner size="lg" label="Loading quotations..." />
          </div>
        ) : error ? (
          ...
          <ZohoEmptyState
            ...
            title={currentSearch || currentStatus ? 'No quotations match your filters' : 'No quotations yet'}
            description={
              currentSearch || currentStatus
                ? 'Try adjusting your search or filter criteria.'
                : 'Create your first quotation to start sending proposals to customers.'
            }
            actionLabel={!currentSearch && !currentStatus ? 'New Quotation' : undefined}
            onAction={
              !currentSearch && !currentStatus
              ...
              <table className="w-full">
                ...
                <tbody>
                  {sortedQuotations.map((quotation, index) => {
                    ...
                    const displayStatus =
                      expired && quotation.status !== 'Expired' && quotation.status !== 'Accepted' && quotation.status !== 'Declined'
                        ? 'Expired'
                        : quotation.status;
                    return (
                      <tr
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm text-[var(--zoho-text)]">
                          {quotation.customer_name || quotation.customer?.display_name || quotation.customer?.company_name || '--'}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <DateCell
                            ...
                            className={expired ? 'text-orange-600 font-medium' : ''}
                          />
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-2 py-3 text-center">
                          <button
                            onClick={(e) => handleDeleteClick(e, quotation)}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--zoho-border)]">
                <p className="text-xs text-[var(--zoho-text-secondary)]">
                  ...
                  {' - '}
                  {Math.min(currentPage * PAGE_SIZE, total)} of {total} quotations
                </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-semibold text-gray-900">Delete Quotation</h3>
                <p className="text-sm text-gray-500 mt-0.5">This action cannot be undone.</p>
              </div>
            </div>
            <p className="text-sm text-gray-700 mb-6">
              Are you sure you want to delete quotation <span className="font-semibold text-gray-900">{deleteConfirm.quotation_number}</span>?
            </p>
