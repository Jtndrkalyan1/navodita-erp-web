...
export default function CustomerFormPage() {
  ...
  const [gstValidation, setGstValidation] = useState({ status: null, data: null, loading: false }); // null | 'valid' | 'invalid'

  // GST validation handler
  const handleValidateGST = async () => {
    const gstin = formData.gstin?.toUpperCase();
    if (!gstin || gstin.length !== 15) {
      toast.error('Please enter a valid 15-character GSTIN before validating');
      return;
    }
    ...
    try {
      ...
      if (result && result.valid) {
        ...
        // Optionally auto-fill address and name from portal
        if (result.legal_name && !formData.display_name) {
          setFormData((prev) => ({ ...prev, display_name: result.legal_name }));
        }
        if (result.legal_name && !formData.company_name) {
          setFormData((prev) => ({ ...prev, company_name: result.legal_name }));
        }
        if (result.state && !billingAddress.state) {
          setBillingAddress((prev) => ({ ...prev, state: result.state }));
        }
        if (result.address && !billingAddress.address_line1) {
          setBillingAddress((prev) => ({ ...prev, address_line1: result.address }));
        }
      } else {
      ...
      setGstValidation({ status: 'invalid', data: null, loading: false });
      toast.error('Failed to validate GST number');
    }
  };
    ...
    const fetchCustomer = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          gst_status: customer.gst_status || '',
          gst_registration_date: customer.gst_registration_date || '',
        });
  ...
  const handleAddressChange = (type, e) => {
    ...
    if (type === 'billing') {
      setBillingAddress((prev) => {
        ...
        // Reset state when country changes since available states differ per country
        if (name === 'country') {
          updated.state = '';
        }
        return updated;
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-[#6B7280]">Loading customer...</span>
        </div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="bg-white rounded-lg border border-[#E5E7EB] shadow-sm p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Customer Type */}
            <div>
              <label className="block text-sm font-medium text-[#333] mb-1">Customer Type</label>
              <select
                name="customer_type"
                ...
                {gstValidation.status === 'invalid' && (
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-red-600 font-medium">
                      Invalid GST Number {gstValidation.data?.status ? `— ${gstValidation.data.status}` : ''}
                    </span>
                  </div>
                )}
                {errors.gstin && (
            ...
            {formData.country === 'India' && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" name="legal_name" value={formData.legal_name} onChange={handleChange} className="w-full px-3 py-2 border border-[#E5E7EB] rounded-lg text-sm text-[#333] placeholder-[#9CA3AF] focus:outline-none focus:ring-2 focus:ring-[#0071DC]/20 focus:border-[#0071DC]" placeholder="As per GST portal" />
                </div>
                <div>
                  ...
                  <input type="text" name="trade_name" value={formData.trade_name} onChange={handleChange} className="w-full px-3 py-2 border border-[#E5E7EB] rounded-lg text-sm text-[#333] placeholder-[#9CA3AF] focus:outline-none focus:ring-2 focus:ring-[#0071DC]/20 focus:border-[#0071DC]" placeholder="Trade name if different" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">Taxpayer Type</label>
                  <input type="text" name="taxpayer_type" value={formData.taxpayer_type} onChange={handleChange} className="w-full px-3 py-2 border border-[#E5E7EB] rounded-lg text-sm text-[#333] placeholder-[#9CA3AF] focus:outline-none focus:ring-2 focus:ring-[#0071DC]/20 focus:border-[#0071DC]" placeholder="e.g. Regular, Composition" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">GST Status</label>
                  <input type="text" name="gst_status" value={formData.gst_status} onChange={handleChange} className="w-full px-3 py-2 border border-[#E5E7EB] rounded-lg text-sm text-[#333] placeholder-[#9CA3AF] focus:outline-none focus:ring-2 focus:ring-[#0071DC]/20 focus:border-[#0071DC]" placeholder="e.g. Active" />
                </div>
              </div>
            )}
