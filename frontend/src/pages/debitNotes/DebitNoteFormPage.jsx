...
function BillDropdown({ bills, value, onChange, error, loading }) {
  return (
    <FormField label="Linked Bill" error={error}>
      <select
        value={value || ''}
      ...
      >
        ...
        {bills.map((bill) => (
          <option key={bill.id} value={bill.id}>
            {bill.bill_number} - {'\u20B9'}{formatIndianNumber(Number(bill.total_amount) || 0)}
          </option>
        ))}
      </select>
    </FormField>
...
export default function DebitNoteFormPage() {
  ...
  // ── Load bills when vendor changes ──────────────────────────────
  ...
  useEffect(() => {
    async function loadBills() {
      ...
      try {
        const res = await billApi.list({ vendor_id: form.vendor_id, limit: 500 });
  ...
  useEffect(() => {
    ...
    async function loadDebitNote() {
      try {
        ...
        const dn = res.data.data || res.data;
        if (!dn) {
        ...
        setForm({
          debit_note_date: dn.debit_note_date || todayISO(),
        ...
        const lineItems = (dn.items || dn.debit_note_items || []).map((item) => ({
          id: item.id,
  ...
  function handleVendorSelect(vendor) {
    if (!vendor) {
      setForm((prev) => ({ ...prev, vendor_id: '', bill_id: '' }));
      setIsDirty(true);
      return;
    }
    setForm((prev) => ({
      ...
      vendor_id: vendor.id,
      bill_id: '',
      place_of_supply: vendor.state || prev.place_of_supply,
    }));
    setIsDirty(true);
    if (errors.vendor_id) {
      setErrors((prev) => {
  ...
  function validate() {
    ...
    if (!form.debit_note_date) {
      errs.debit_note_date = 'Debit note date is required';
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isEdit) {
        await debitNoteApi.update(id, payload);
        toast.success('Debit note updated successfully');
      } else {
        const res = await debitNoteApi.create(payload);
        const newId = res.data?.data?.id || res.data?.id;
        toast.success('Debit note created successfully');
        setIsDirty(false);
        navigate(newId ? `/debit-notes/${newId}` : '/debit-notes', { replace: true });
        return;
      }
      setIsDirty(false);
      navigate(`/debit-notes/${id}`, { replace: true });
    } catch (err) {
      const msg = err.response?.data?.message || err.response?.data?.detail || 'Failed to save debit note';
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  function handleCancel() {
    if (confirmLeave()) {
      navigate(isEdit ? `/debit-notes/${id}` : '/debit-notes');
    }
  }
      ...
      <div className="bg-white border-b border-[var(--zoho-border)] px-6 py-4 -mx-6 -mt-6 mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-[var(--zoho-text)]">
            {isEdit ? `Edit Debit Note${form.debit_note_number ? ` #${form.debit_note_number}` : ''}` : 'New Debit Note'}
          </h1>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg border border-[var(--zoho-border)] p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-1">
            ...
            <DatePicker
              ...
              onChange={(v) => updateForm('debit_note_date', v)}
              required
              error={errors.debit_note_date}
            />

            {/* Debit Note Number */}
            <FormField label="Debit Note Number">
              <input
                type="text"
                value={form.debit_note_number}
                onChange={(e) => updateForm('debit_note_number', e.target.value)}
            ...
            <BillDropdown
              bills={bills}
              value={form.bill_id}
              onChange={(v) => updateForm('bill_id', v)}
            ...
            <div className="lg:col-span-2">
              <FormField label="Reason">
                <textarea
                  ...
                  rows={2}
                  placeholder="Reason for the debit note (e.g., defective goods, pricing discrepancy)"
