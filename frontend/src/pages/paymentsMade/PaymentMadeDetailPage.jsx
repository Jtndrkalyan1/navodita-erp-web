...
export default function PaymentMadeDetailPage() {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await paymentMadeApi.remove(id);
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate('/payments-made')}
          ...
          <div>
            ...
            <p className="text-sm text-[#6B7280] mt-0.5">
              Payment to {payment.vendor_name || payment.vendor?.display_name || 'Unknown'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => printPdf(`/api/pdf/payment-made/${id}`)}
          ...
          <button
            onClick={() => openPdf(`/api/pdf/payment-made/${id}`)}
            className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--zoho-text-secondary)] bg-white border border-[var(--zoho-border)] px-3 py-2 rounded-md hover:bg-gray-50 transition-colors cursor-pointer"
          >
          ...
          <ShareButton
            ...
            documentId={id}
            recipientName={payment.vendor_name || payment.vendor?.display_name || ''}
            recipientEmail={payment.vendor?.email || ''}
            recipientPhone={payment.vendor?.phone || payment.vendor?.mobile || ''}
            amount={payment.amount}
            date={payment.payment_date}
            pdfUrl={`/api/pdf/payment-made/${id}`}
          />
          <Link
            to={`/payments-made/${id}/edit`}
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] shadow-sm p-6 mb-6">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-lg bg-orange-100 flex items-center justify-center">
            <HiOutlineBanknotes className="w-7 h-7 text-orange-600" />
          </div>
          <div>
            <p className="text-xs text-[#6B7280] font-medium uppercase tracking-wide">Amount Paid</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-lg border border-[#E5E7EB] shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1">
              ...
              <InfoRow label="Vendor" value={payment.vendor_name || payment.vendor?.display_name} />
          ...
          {allocations.length > 0 && (
            <div className="bg-white rounded-lg border border-[#E5E7EB] shadow-sm p-6">
              <h2 className="text-base font-semibold text-[#333] mb-4">Bill Allocations</h2>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-[#6B7280] mt-1">
                  ...
                  This will also remove all bill allocations. This action cannot be undone.
                </p>
              </div>
            </div>
