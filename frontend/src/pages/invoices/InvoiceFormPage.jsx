        ...
        {isOpen && (
          <div className="absolute z-50 mt-1 w-full bg-white border border-[var(--zoho-border)] rounded-lg shadow-lg max-h-72 overflow-hidden">
            ...
            <div className="border-t border-gray-100">
              <button
                type="button"
                onClick={() => { setIsOpen(false); navigate('/customers/new'); }}
  ...
  useEffect(() => {
    async function loadCustomers() {
      try {
        ...
        // Customers will be empty; user can still type
        setCustomers([]);
      } finally {
    ...
    async function loadInvoice() {
      try {
        ...
        setForm({
          ...
          bill_to_attention: inv.bill_to_attention || '',
          bill_to_address_line1: inv.bill_to_address_line1 || '',
          bill_to_address_line2: inv.bill_to_address_line2 || '',
          bill_to_city: inv.bill_to_city || '',
          bill_to_state: inv.bill_to_state || '',
          bill_to_pincode: inv.bill_to_pincode || '',
          bill_to_country: inv.bill_to_country || 'India',
          ship_to_attention: inv.ship_to_attention || '',
          ship_to_address_line1: inv.ship_to_address_line1 || '',
          ship_to_address_line2: inv.ship_to_address_line2 || '',
          ship_to_city: inv.ship_to_city || '',
          ship_to_state: inv.ship_to_state || '',
          ship_to_pincode: inv.ship_to_pincode || '',
          ship_to_country: inv.ship_to_country || 'India',
        ...
        // Load line items
        const lineItems = (inv.items || inv.invoice_items || []).map((item) => ({
          id: item.id,
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isEdit) {
        ...
        setIsDirty(false);
        navigate(newId ? `/invoices/${newId}` : '/invoices', { replace: true });
        return;
      }
      setIsDirty(false);
      navigate(`/invoices/${id}`, { replace: true });
    } catch (err) {
      const msg = err.response?.data?.message || err.response?.data?.detail || 'Failed to save invoice';
      toast.error(msg);
    } finally {
      setSaving(false);
    }
    ...
    <form onSubmit={handleSubmit} className="pb-8">
      ...
      <div className="bg-white border-b border-[var(--zoho-border)] px-6 py-4 -mx-6 -mt-6 mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-[var(--zoho-text)]">
            {isEdit ? `Edit Invoice${invoiceNumber ? ` #${invoiceNumber}` : ''}` : 'New Invoice'}
          </h1>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-[var(--zoho-border)] p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-1">
            ...
            <SelectField
              ...
              onChange={(v) => updateForm('currency_code', v)}
              options={[
        ...
        <div className="bg-white rounded-lg border border-[var(--zoho-border)] p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <FormField label="Customer Notes">
              <textarea
                ...
                onChange={(e) => updateForm('customer_notes', e.target.value)}
                rows={3}
          ...
          <FormField label="Internal Notes" className="mt-2">
            <textarea
